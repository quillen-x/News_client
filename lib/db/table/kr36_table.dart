...
class Kr36Table extends TableOperation {
  Future<List<Kr36DetailModel>> query() async {
    final data = await dDatabase.query(
      DSTableDefine.kr36Table,
      orderBy: '"create" DESC',
      limit: DSTableDefine.maxHotRecords,
    );
    return List.generate(data.length, (index) {
      return Kr36DetailModel.fromJson(data[index]);
    });
  }

  Future<void> insertHotBatch(List<Kr36DetailModel> list) async {
  ...
  Future<void> _trimToMax() async {
    await dDatabase.rawDelete('''
      DELETE FROM ${DSTableDefine.kr36Table}
      WHERE id NOT IN (
        SELECT id FROM ${DSTableDefine.kr36Table}
